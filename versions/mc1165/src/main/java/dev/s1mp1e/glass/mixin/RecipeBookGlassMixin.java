...
/**
 * System 2 (recipe book glass) — the 1.16.5 Fabric port of LiquidGlass26's
 ...
 *       the bar. This is the signature 26.2 detail the earlier one-glass-per-tab
 *       port was missing.</li>
 * </ol>
 *
 * <h3>Injection point (verified from bytecode)</h3>
 * {@code RecipeBookWidget} (obf {@code drp} / {@code class_507}) overrides
 * {@code render} (yarn {@code method_25394},
 * {@code (Lnet/minecraft/client/util/math/MatrixStack;IIF)V}). Its first draw blits
 * the book PNG via {@code drawTexture(MatrixStack,IIIIII)} — the invoke's OWNER is
 * {@code RecipeBookWidget} itself (javac emits the receiver's static type), so the
 * {@code @Redirect} owner is {@code RecipeBookWidget}. {@code drawTexture} is a
 * PUBLIC method inherited from {@code DrawableHelper}; it is NOT {@code @Shadow}'d
 * (that throws "not located in target class" for inherited members) — the no-glass
 * fallback calls it through the redirect's {@code self} param instead.
 *
 * <h3>Tab-bar geometry (26.2, byte-for-byte)</h3>
 * bbox of VISIBLE tabs only (hidden tabs sit at (0,0) and would drag the bbox to
 * the screen edge): bar {@code [minX+1, minX+28]} (27px wide, right edge still 2px
 * short of the book) × {@code [firstTabTop, lastTabBottom]}; the pill clears the bar
 ...
 * (unlike the inherited {@code drawTexture}).
 */
