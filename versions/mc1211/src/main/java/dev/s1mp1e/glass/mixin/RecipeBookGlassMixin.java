...
/**
 * System 2 (recipe book glass) — the 1.20.1 Fabric port of LiquidGlass26's
 * {@code RecipeBookGlassMixin}. Reproduces the full 26.2 look, which is THREE
 * things, not just the pop-out background:
 *
 * <ol>
 ...
 * </ol>
 *
 * <h3>Injection point (verified from 1.17.1 bytecode, yarn 1.17.1+build.65)</h3>
 * {@code RecipeBookWidget} overrides {@code render}
 * ({@code (Lnet/minecraft/client/util/math/MatrixStack;IIF)V}). Its ONLY
 * {@code drawTexture(MatrixStack,IIIIII)} INVOKE (offset 86, constant-pool
 * {@code #585 = RecipeBookWidget.drawTexture:(Lnet/minecraft/client/util/math/MatrixStack;IIIIII)V})
 * blits the book PNG — the invoke OWNER is {@code RecipeBookWidget} itself (javac
 * emits the receiver's static type), so the {@code @Redirect} owner is
 * {@code RecipeBookWidget}. {@code drawTexture} is a PUBLIC method inherited from
 * {@code DrawableHelper}; it is NOT {@code @Shadow}'d (that throws "not located in
 * target class" for inherited members) — the no-glass fallback calls it through the
 * redirect's {@code self} param instead.
 *
 * <p>The {@code tabButtons} {@code List} and the {@code currentTab} field are both
 * declared ON {@code RecipeBookWidget} (verified: {@code private final
 * List<RecipeGroupButtonWidget> tabButtons} / {@code private RecipeGroupButtonWidget
 * currentTab}), so {@code @Shadow} of them resolves cleanly.
 */
...
public abstract class RecipeBookGlassMixin {
    ...
    private void s1mp1e$glassBook(DrawContext self, Identifier texture,
                                  int x, int y, int u, int v, int width, int height) {
        // No glass path on this GPU -> draw the vanilla book PNG unchanged.
        if (!GlassProgram.ensureReady() || !GlassProgram.usable()) {
            self.drawTexture(texture, x, y, u, v, width, height);
            return;
        }
     ...
     * {@code RecipeBookGlassMixin#lg$glassBook} lines 56-104.
     */
    private void s1mp1e$drawTabBar(float fade) {
        ...
        if (currentTab != null) {
            ...
            int py1v = Math.round(Math.max(s1mp1e$py1.value(), s1mp1e$py2.value())) + tabH - 2;
