 ...
 * Clicks arrive from {@code MouseClickMixin} (GLFW press on {@code Mouse
 * .onMouseButton}) so every hardware press is seen even when several land in one
...
public final class CpsModule extends Module implements HudBounds, HudRenderer {
    ...
    public static void recordClick(int button) {
        ...
        if (mc.currentScreen != null || mc.player == null) return;   // in-world clicks only
    ...
    public void renderHud(DrawContext ctx) {
        ...
        GlassFont.drawARGB(ctx, text, posX.intValue, posY.intValue, color.colorValue, shadow.boolValue);
    }
