 ...
 * alpha bar). The SV square is drawn as per-column VERTICAL gradients (1.21.1 fillGradient
...
public final class ColorWidget extends Widget {
    ...
    @Override public void draw(DrawContext ctx, int mouseX, int mouseY, float pt, float alphaF) {
        ...
        for (int r = 0; r < rows; r++)
            for (int c = 0; c < cols; c++) {
                ...
                GlassWidgets.fill(ctx, qx, qy, Math.min(qx + cs, x1), Math.min(qy + cs, y1), ((r + c) % 2 == 0) ? light : dark);
            }
        int cur = s.colorValue;
        int ca = (cur >>> 24) & 0xFF; if (ca == 0) ca = 255;
        GlassWidgets.fillRound(ctx, x, y0, x1, y1, (Math.round(ca * alphaF) << 24) | (cur & 0xFFFFFF), 3f);
        GlassWidgets.border(ctx, x, y0, x1, y1, (Math.round(a * (open ? 0.9f : 0.3f)) << 24) | (open ? 0x0A84FF : 0xFFFFFF));
    }

    @Override public void drawOverlay(DrawContext ctx, int mouseX, int mouseY, float alphaF) {
        if (!open) return;
        float px = pX0(), py = pY0();
        float ph = PAD * 4 + SVH + BARH * 2;
        // rounded panel (border ring behind + dark body)
        GlassWidgets.fillRound(ctx, px - PAD - 1, py - PAD - 1, px + PW + PAD + 1, py + ph - PAD + 1, 0x600A84FF, 8f);
        GlassWidgets.fillRound(ctx, px - PAD, py - PAD, px + PW + PAD, py + ph - PAD, 0xF01C1C1E, 7f);
        ...
        for (int i = 0; i < PW; i += STEP) {
            ...
            GlassWidgets.gradientV(ctx, px + i, svy0, Math.min(px + i + STEP, px + PW), svy1, top, 0xFF000000);
        }
        GlassWidgets.border(ctx, px, svy0, px + PW, svy1, 0x30FFFFFF);
        // SV marker
        float mx0 = px + S * PW, my0 = svy1 - V * SVH;
        GlassWidgets.border(ctx, mx0 - 3, my0 - 3, mx0 + 3, my0 + 3, 0xFFFFFFFF);

        // --- hue bar ---
        float hy0 = svy1 + PAD, hy1 = hy0 + BARH;
        for (int i = 0; i < PW; i += STEP) {
            int c = 0xFF000000 | hsvRgb(i / PW, 1f, 1f);
            GlassWidgets.fill(ctx, px + i, hy0, Math.min(px + i + STEP, px + PW), hy1, c);
        }
        GlassWidgets.fill(ctx, px + H * PW - 1, hy0 - 1, px + H * PW + 1, hy1 + 1, 0xFFFFFFFF);
        ...
        for (int i = 0; i < PW; i += STEP) {
            ...
            GlassWidgets.fill(ctx, px + i, ay0, Math.min(px + i + STEP, px + PW), ay1, chk ? 0xFF808080 : 0xFF545454);
            int al = Math.round(i / PW * 255f);
            GlassWidgets.fill(ctx, px + i, ay0, Math.min(px + i + STEP, px + PW), ay1, (al << 24) | rgb);
        }
        GlassWidgets.fill(ctx, px + A * PW - 1, ay0 - 1, px + A * PW + 1, ay1 + 1, 0xFFFFFFFF);
    }
