...
public final class GlassProgram {
    ...
    public static void bind(int kind) {
        ...
        if (uScreen[kind]   >= 0) GL20.glUniform2f(uScreen[kind], s1mp1e$vpW(), s1mp1e$vpH());
    ...
    private static String read(String domain, String path) throws Exception {
        ...
        // straight off the classpath instead of MC's ResourceManager.
    ...
    // 1.13.2: mc.window is unmapped in Legacy Fabric yarn; the GUI viewport
    // equals the framebuffer, so read it straight from GL.
