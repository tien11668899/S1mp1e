...
public final class GlassRenderer {
    ...
    public static void endBatch() {
        ...
        // CRITICAL: batchQuad sets the vertex colour with RAW GL11.glColor4f, which
        // bypasses GlStateManager's colour cache. If we reset through
        // GlStateManager.color4f here, its cache still thinks the colour is white
        // (it never saw the raw calls) and NO-OPs — leaving the real GL colour at the
        // LAST quad's value. The lattice's empty-corner cell ends on alpha 0, so every
        // vanilla draw AFTER the glass this frame (the survival player model via
        // enableColorMaterial, item stacks, etc.) gets multiplied to invisible. Reset
        // with a RAW glColor4f so the real GL colour is actually white again.
