...
// Liquid Glass for MC 26.2 — LENS variant of glass.fsh for the switch knob / slider thumb.
// Same reglass/iyinchao refraction, two differences from glass.fsh:
//   * CORNER_FRAC = 1.0 so the vertex-RED corner can reach a full capsule (glass.fsh caps at 0.5·min,
...
//     keeps a clear magnifying centre with the bending concentrated at its rim, instead of the whole
//     lens sitting inside one 13px band.
// Everything else — Snell edge, dispersion, Fresnel rim, drop shadow, the R/G/B/A knobs — is unchanged,
// so passing lift = 0 (G = 255) keeps the rim subtle (no selector amplification).
...
const float CORNER_FRAC   = 1.0;     // LENS: allow the vertex-RED corner to reach a full capsule
const float REF_THICKNESS_MAX = 13.15;   // refraction band cap, px (scaled to the element below)
...
void main() {
    ...
    // LENS: refraction band proportional to the element so a small thumb keeps a clear centre
    float REF_THICKNESS = max(min(min(halfPx.x, halfPx.y) * 0.9, REF_THICKNESS_MAX), 0.5);
