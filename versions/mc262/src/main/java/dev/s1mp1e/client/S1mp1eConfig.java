...
public final class S1mp1eConfig {
    ...
    private static int menuKey = GLFW.GLFW_KEY_RIGHT_SHIFT;   // GLFW 344 (1.21.1 uses GLFW codes)
    ...
    private static File configFile() {
        ...
        base = (mc != null && mc.gameDirectory != null) ? mc.gameDirectory : new File(".");
        // SHARED across versions: the S1mp1e launcher runs every version from .minecraft/instances/<mc>/, so
        // the common .minecraft/s1mp1e-mods/ folder holds ONE modules.json that every version reads AND
        // writes -> settings stay in sync between versions. Falls back to the per-instance config when that
        // shared folder isn't present (dev runClient, or a non-launcher launch).
        ...
        if (instances != null) {
            ...
            if (dotMinecraft != null) {
                File shared = new File(dotMinecraft, "s1mp1e-mods");
                if (shared.isDirectory()) return new File(shared, FILE_NAME);
            }
        }
    ...
    private static JsonObject build() {
        // MERGE, don't replace: modules.json is SHARED by every version (1.20.1, 1.21.1, 26.2 ...) and this
        // build may register fewer modules than another one (26.2 is being ported phase by phase). Starting
        // from the file's existing "modules" object keeps entries for modules this build doesn't know, so a
        // save here can never wipe another version's settings; registered modules overwrite their own entry.
        JsonObject modules = readExistingModules();
    ...
    /** The "modules" object currently on disk, or an empty one if the file is missing/unreadable. */
    private static JsonObject readExistingModules() {
        ...
        try {
            ...
            if (root != null && root.isJsonObject()) {
                JsonElement el = root.getAsJsonObject().get("modules");
