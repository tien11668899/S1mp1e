...
public final class ModeWidget extends Widget {
    ...
    @Override public void draw(GuiGraphicsExtractor g, int mouseX, int mouseY, float pt, float alpha) {
        ...
        GlassWidgets.capsule(g, cx0, y0, x1, y1, 0.5f, hover ? 0.7f : 0.35f, alpha, true);
        float cy = (y0 + y1) / 2f;
        GlassWidgets.label(g, Lang.mode(s.modeValue), cx0 + 10f, cy - GlassWidgets.fontH() / 2f, 0xF5F5F7, alpha);
        GlassWidgets.label(g, ">", x1 - 11f, cy - GlassWidgets.fontH() / 2f, 0x8E8E93, alpha);
    }
