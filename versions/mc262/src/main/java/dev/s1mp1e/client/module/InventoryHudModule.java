 ...
 *
 * <p><b>26.2 port.</b> {@code DrawContext.drawItem/drawItemInSlot} → {@code g.item/g.itemDecorations}; the
 * mc1211 raw-GL shader/lighting priming after the glass is gone (26.2 is deferred — the glass is just an
 * earlier render state, it can't corrupt item state). {@code inventory.main} → {@code getNonEquipmentItems()}
 * (same 0-8 hotbar / 9-35 main layout). {@code currentScreen} → {@code mc.gui.screen()}.
 *
 * <p><b>Fair play.</b> Reads the local player's own inventory — information you already have.
 * OBSERVE-ONLY: it never moves, swaps, uses, or drops anything, and only polls whether the peek key is held.
 */
public final class InventoryHudModule extends Module implements HudBounds, HudRenderer {
    ...
    private int effX() {
        ...
        return (Minecraft.getInstance().getWindow().getGuiScaledWidth() - panelW()) / 2;
    }
