...
/**
 * In-game liquid-glass config screen — 26.2 port of the mc1211 screen.
 *
 * <p>26.2 changes (layout, tuning and behaviour are otherwise identical to mc1211):
 * <ul>
 *   <li>{@code render(DrawContext)} → {@link #extractRenderState}; vanilla now extracts the blurred background
 *       itself (in the stratum before ours), so there is no renderBackground / flush / backdrop re-grab here —
 *       the glass pipeline's backdrop is grabbed once per frame by {@code GuiRendererGrabMixin}.</li>
 *   <li>input arrives as {@link MouseButtonEvent} / {@link KeyEvent} / {@link CharacterEvent} records and is
 *       unpacked to the same ints the widgets used before;</li>
 *   <li>{@code shouldPause → isPauseScreen}, {@code close → onClose}, {@code client → minecraft.gui}.</li>
 * </ul>
 */
public final class S1mp1eConfigScreen extends Screen {
    ...
    private void selectModule(Module m) {
        ...
        if (selected == null || m == selected) {        // first pick / re-pick: no fade
            ...
            pendingSelect = m; detailFade.to(0f);        // fade the detail out; extractRenderState() swaps once faded
        }
    }
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        // (26.2: vanilla already extracted the blurred background in the previous stratum.)
        ...
        GlassWidgets.fill(g, railX1, py0 + PAD, railX1 + 1, py1 - PAD, div);            // vertical rail
        GlassWidgets.fill(g, px0 + PAD, listY0 - GAP, railX1 - PAD, listY0 - GAP + 1, div); // under tabs
        GlassWidgets.fill(g, detailX0, setY0 - GAP, px1 - PAD, setY0 - GAP + 1, div);       // under detail title
        ...
        GlassWidgets.capsule(g, hx0 + 2, tcy - TAB_PILL_H / 2f, hx0 + tw - 2, tcy + TAB_PILL_H / 2f, 1f, 0.6f, a, true);
        for (int i = 0; i < 3; i++) {
            float[] r = tabRect[i];
            float prox = Math.max(0f, 1f - Math.abs(tp - i));
            GlassWidgets.label(g, TAB_LABELS[i], r[0] + (r[2] - r[0] - GlassWidgets.strW(TAB_LABELS[i])) / 2f,
        ...
        GlassWidgets.enableScissor(g, (int) px0, (int) listY0, (int) railX1, (int) listY1);
        ...
        if (!modules.isEmpty()) {
            ...
            GlassWidgets.capsule(g, px0 + PAD, sr0, railX1 - PAD, sr1, 0.4f, 0.5f, pa, true);
        }
        ...
        for (int i = 0; i < rows; i++) {
            ...
            GlassWidgets.label(g, Lang.module(m.name), px0 + PAD + ROW_PILL_M, (r[1] + r[3]) / 2f - GlassWidgets.fontH() / 2f,
                    lerpRGB(0xE0E0E6, 0xFFFFFF, prox), pa);
            moduleToggles.get(i).draw(g, mouseX, mouseY, delta, pa);
        }
        GlassWidgets.disableScissor(g);

        // detail (page content at pa)
        if (selected != null) {
            GlassWidgets.label(g, Lang.module(selected.name), detailX0,
            ...
            drawChip(g, "重置全部", resetAllRect, mouseX, mouseY, pa, 0xFFFFFF);
            drawChip(g, "編輯 HUD", editHudRect, mouseX, mouseY, pa, 0x0A84FF);
            // ...but 編輯排版 is module-specific, so it fades in/out with the detail (da).
            if (selected instanceof LayoutEditable) drawChip(g, "編輯排版", layoutRect, mouseX, mouseY, da, 0x30D158);
            GlassWidgets.enableScissor(g, (int) detailX0, (int) setY0, (int) (px1 - PAD), (int) setY1);
            for (int i = 0; i < settingWidgets.size(); i++) {
                Setting s = shownSettings.get(i);
                Widget w = settingWidgets.get(i);
                float cy = (w.y0 + w.y1) / 2f;
                GlassWidgets.label(g, Lang.setting(s.name), detailX0, cy - GlassWidgets.fontH() / 2f, 0xC7C7CC, da);
                w.draw(g, mouseX, mouseY, delta, da);
            }
            GlassWidgets.disableScissor(g);
        }

        // iOS-26 scroll-edge over each list's top/bottom; each end fades in with how far
        // that end can still scroll. (26.2: dim-only — see GlassWidgets.scrollEdges.)
        // EXT = 2× the panel corner radius (~14) so the fade band matches the panel corner.
        final float EXT = 28f;
        float modMax = Math.max(0f, modules.size() * ROW_MOD - (listY1 - listY0));
        GlassWidgets.scrollEdges(g, px0 + HAIR, listY0, railX1 - HAIR, listY1, EXT,
                                 modScroll / EXT, (modMax - modScroll) / EXT, a);
        if (selected != null) {
            float setMax = Math.max(0f, settingWidgets.size() * ROW_SET - (setY1 - setY0));
            GlassWidgets.scrollEdges(g, detailX0, setY0, px1 - PAD, setY1, EXT,
                                     setScroll / EXT, (setMax - setScroll) / EXT, a);
        }

        // widget overlays (colour picker popup) LAST + unclipped, in their own stratum so they sit on top.
        if (selected != null) {
            g.nextStratum();
            for (Widget w : settingWidgets) w.drawOverlay(g, mouseX, mouseY, da);
        }
    }

    private void drawChip(GuiGraphicsExtractor g, String text, float[] r, int mx, int my, float a, int rgb) {
        boolean hover = GlassWidgets.inside(mx, my, r[0], r[1], r[2], r[3]);
        GlassWidgets.capsule(g, r[0], r[1], r[2], r[3], 0.5f, hover ? 0.7f : 0.3f, a, true);
    ...
    @Override public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        try {
            return s1mp1e$routeClick(event, doubleClick);
        } finally {
            for (Widget w : settingWidgets) w.clickDispatched();
        }
    }

    private boolean s1mp1e$routeClick(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (selected != null) {
            if (selected instanceof LayoutEditable && hit(layoutRect, mx, my)) {
                this.minecraft.gui.setScreen(((LayoutEditable) selected).openLayoutEditor()); return true;
            }
            if (hit(editHudRect, mx, my)) { this.minecraft.gui.setScreen(new S1mp1eHudEditScreen()); return true; }
            ...
            if (GlassWidgets.inside(mx, my, detailX0, setY0, px1 - PAD, setY1)) {
                for (Widget w : settingWidgets) if (w.mouseClickedPrecise(event.x(), event.y(), btn)) return true;
                return true;
            }
        }
    ...
    @Override public boolean mouseScrolled(double mxd, double myd, double dx, double dy) {
        int mx = (int) mxd, my = (int) myd;
        float step = (float) (-dy * ROW_SET);
        if (GlassWidgets.inside(mx, my, px0, listY0, railX1, listY1)) {
    ...
    @Override public boolean keyPressed(KeyEvent event) {
        ...
        if (event.isEscape() || keyCode == S1mp1eConfig.getMenuKey()) { this.onClose(); return true; }
    ...
    // Don't dismiss instantly — start the fade-out; extractRenderState() calls super.onClose() once
